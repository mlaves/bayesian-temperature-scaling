...
# python -u train.py --batch_size=${BS} --epochs=500 --model=densenet121 --label_smoothing=0.0 --dataset=cifar10 | tee `date '+%Y-%m-%d_%H-%M-%S'`_densenet121_smooth.log
# python -u train.py --batch_size=${BS} --epochs=500 --model=densenet121 --label_smoothing=0.1 --dataset=cifar10 | tee `date '+%Y-%m-%d_%H-%M-%S'`_densenet121_smooth.log
